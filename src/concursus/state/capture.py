...
FZ 35e4a3a1b SPIKE A. Hive's memory store (:mod:`concursus.state.filevault` +
...
# Sources named by the design (FZ 35e4a3a1a1) but not yet wired to a seam — declaring them keeps
...
def adapt_plan(plan: Any, run_dir: str, *, trail_id: str = "run", date: str = "") -> CaptureEnvelope:
    ...
    FZ 35e5a #1 (persist the frozen plan) through the envelope/dispatcher seam.
    """
...
def adapt_payload(
    ...
    *,
    ...
    """Adapt a node's frozen invoke PAYLOAD into a :data:`PAYLOAD` envelope (FZ 35e4a3a1b T3).
    ...
    ``payload``-stamped non-record slipbox note. Realizes FZ 35e5a #2 (persist per-agent payloads).
    """
...
def capture_run(
    ...
    *,
    ...
    """**FZ 35e4a3a1b T4.** The post-run capture trigger: build + dispatch envelopes for a finished
...
def gate_run_dir(run_dir: str) -> Dict[str, Any]:
    """**FZ 35e4a3a1b T5.** Run a lightweight post-write GATE over a run's capture notes; return a
    ...
    not on disk. This is the [FZ 35e4a3a1a1a #2] "actually RUN the format/link check, don't just
...
def load_payload_tiers(run_dir: str) -> Dict[str, str]:
    """Read persisted payload notes back into ``{node: trust_tier}`` (FZ 35e4a3a1b I2 — FUTURE hook).

    The minimal read-back primitive the FZ 35e5a P3/P4 flywheel needs: it re-reads the
