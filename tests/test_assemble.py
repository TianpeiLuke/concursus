...
# -- Phase 4: wire compile_next -> recompile (close the dead channel) --
...
# -- B2: OrchestrationAssembler(strict_types=) threads the deep gate ----------
...
# -- B1: OrchestrationAssembler(single_writer=) threads the non-overlap gate ----------
...
# -- B4: strict_fn dials the deep gate per node -------------------
...
# -- OPT-IN validate-and-retry budget hook ----------------------------------
from concursus.assemble.assemble import DEFAULT_MAX_REVISIONS
from concursus.execute.supervisor import SchemaError


def _schema_agent(required_field="report"):
    """A manifest whose output schema requires one field (the retry gate)."""
    return _agent(
        "worker",
        {"task": {"type": "string"}},
        {"properties": {required_field: {"type": "string"}}, "required": [required_field]},
    )


def test_retry_budget_clamps_to_revision_ceiling():
    """The retry budget shares the recompile ceiling: negatives floor at 0, over-cap is clamped."""
    assert OrchestrationAssembler.retry_budget(-5) == 0
    assert OrchestrationAssembler.retry_budget(3) == 3
    assert OrchestrationAssembler.retry_budget(DEFAULT_MAX_REVISIONS + 100) == DEFAULT_MAX_REVISIONS
    assert OrchestrationAssembler.retry_budget(2, max_revisions=1) == 1


def test_redrive_returns_first_valid_output_without_retry():
    """A first-attempt schema-valid output returns immediately (drive_fn called exactly once)."""
    manifest = _schema_agent()
    calls = []
    ...
    result = OrchestrationAssembler().redrive_until_valid("worker", manifest, drive, max_retries=3)
    assert result == {"report": "ok"}
    assert len(calls) == 1  # no retry needed
    assert "validation_error" not in calls[0]  # first attempt gets no error context


def test_redrive_feeds_error_back_and_recovers():
    """A failing first output re-drives with the SchemaError fed back into the next context."""
    manifest = _schema_agent()
    contexts = []

    def drive(context):
        contexts.append(dict(context))
        # First attempt omits the required field; second attempt (seeing the error) supplies it.
        return {"report": "fixed"} if "validation_error" in context else {"wrong": "x"}

    result = OrchestrationAssembler().redrive_until_valid("worker", manifest, drive, max_retries=1)
    assert result == {"report": "fixed"}
    assert len(contexts) == 2
    assert "validation_error" in contexts[1]
    assert "worker" in contexts[1]["validation_error"]


def test_redrive_raises_after_budget_exhausted():
    """A node that never conforms raises the last SchemaError after 1 + budget bounded drives."""
    manifest = _schema_agent()
    calls = []

    def drive(context):
        calls.append(1)
        return {"wrong": "x"}  # always invalid

    with pytest.raises(SchemaError, match="missing required"):
        OrchestrationAssembler().redrive_until_valid("worker", manifest, drive, max_retries=2)
    assert len(calls) == 3  # 1 initial + 2 retries (bounded, not unbounded)


def test_redrive_budget_bounded_by_revision_cap():
    """max_retries above the revision cap is clamped: drives never exceed 1 + max_revisions."""
    manifest = _schema_agent()
    calls = []
    ...
    with pytest.raises(SchemaError):
        OrchestrationAssembler().redrive_until_valid(
            "worker", manifest, drive, max_retries=999, max_revisions=2
        )
    assert len(calls) == 3  # 1 initial + clamp(999, 2) = 2 retries


def test_redrive_does_not_mutate_base_context():
    """The caller's base_context is copied, never mutated by the error feedback."""
    manifest = _schema_agent()
    base = {"task": "summarize"}
    ...
    OrchestrationAssembler().redrive_until_valid(
        "worker", manifest, drive, base_context=base, max_retries=1
    )
    assert base == {"task": "summarize"}  # untouched


def test_assemble_default_output_unchanged_by_new_helper():
    """The retry helper is inert on assemble(): the default compiled plan is byte-for-byte unchanged."""
    dag, manifests = _chain()
    plan = OrchestrationAssembler().assemble(dag, manifests)
    assert plan.revision == 0
    assert plan.frontier == []
    assert "revision" not in plan.to_dict()
