...
class AlignmentError(ValueError):
    ...
    Every rejection is ANSWER-CARRYING: besides the human message it carries STRUCTURED
    attributes so a programmatic caller (e.g. the ``staff_with_rebind`` re-binder) can react
    WITHOUT parsing the message text:

    - ``node`` — the offending consumer node (``None`` when not node-specific).
    - ``producer`` — the implicated producer node (``None`` when not edge-specific).
    - ``field`` — the offending field: the referenced output field, the consumer input name, or
      the capability kind, depending on the rejection (``None`` when not applicable).
    - ``expected`` — the expected type/shape (e.g. the consumer input's declared type, or the set
      of still-missing required capabilities) (``None`` when not applicable).
    - ``candidates`` — the set of VALID alternatives the caller could pick from (e.g. the
      producer's declared output fields, the consumer's declared inputs, the known producer ids,
      or the capabilities the agent actually declares) (``None`` when not applicable).

    All attributes default to ``None``; the ``node``/``producer`` pair predates the others
    ( C2) and their values on existing rejections are unchanged.
    """

    def __init__(self, message: str, *, node: "Optional[str]" = None,
                 producer: "Optional[str]" = None, field: "Optional[str]" = None,
                 expected: Any = None,
                 candidates: "Optional[tuple]" = None) -> None:
...
# -- required-capability extraction (opt-in gate G6) ------------------------
_CAP_KINDS = ("features", "tools", "egress_hosts")


def _required_capabilities(manifest: Any) -> Dict[str, Set[str]]:
    """Author-declared REQUIRED capabilities of ``manifest``, read tolerantly (empty when absent).

    Looks for a ``spec.requires`` block — the mirror of the ``capabilities`` inventory:
    ``{"features": [...], "tools": [...], "egress_hosts": [...]}``. A bare list/tuple is treated
    as ``features`` (the common one-dimension shorthand). Returns a ``{kind: set(labels)}`` map over
    the three capability kinds; an absent/empty ``requires`` yields three empty sets, so a manifest
    that declares nothing required imposes no gate. Never raises — a malformed value simply
    contributes nothing for that kind (the gate can only PROVE a gap, never guess one).
    """
    empty = {kind: set() for kind in _CAP_KINDS}
    ...
    if isinstance(requires, (list, tuple, set)):
        return {**empty, "features": {str(x) for x in requires}}
...
def _declared_capabilities(manifest: Any) -> Dict[str, Set[str]]:
    """The capability inventory an agent's runtime actually declares, as a ``{kind: set}`` map.

    Reads the typed :class:`~concursus.core.manifest.AgentCapabilities` block off the
    manifest; the empty default declares nothing, so an un-annotated agent covers nothing.
    """
    caps = getattr(manifest, "capabilities", None)
    return {kind: set(getattr(caps, kind, ()) or ()) for kind in _CAP_KINDS}
...
def check_alignment(
    ...
    *,
    ...
    ``strict_types`` (default ``False``) adds a DEEPER gate ( B2): the producer output
    ...
    ``single_writer`` (default ``False``) adds the NON-OVERLAP gate ( B1): no consumer
    ...
    ``strict_fn`` (default ``None``, B4 — the ADAPTIVE STRICTNESS DIAL) NARROWS the deep
    ...
    ``full_input_cover`` (default ``False``, F2 — the b2 dimension-1 COMPLETENESS
    ...
    ``require_capabilities`` (default ``False``) adds the COMPILE-TIME CAPABILITY gate: when a
    manifest declares REQUIRED capabilities (an author-declared ``spec.requires`` block mirroring the
    ``capabilities`` inventory — ``{features?, tools?, egress_hosts?}``, or a bare list read as
    ``features``), the agent's own ``manifest.capabilities`` block MUST declare every one, or the
    compile fails with an answer-carrying :class:`AlignmentError` (``field`` = the capability kind,
    ``expected`` = the still-missing labels, ``candidates`` = the labels the agent actually
        ...
        for edge in manifest.depends_on:
            ...
            if field not in properties:
                raise AlignmentError(
                    ...
                    producer=producer,
                    field=field,
                    candidates=tuple(sorted(properties)),
                )
            ...
            if input_name not in consumer_inputs:
                raise AlignmentError(
                    ...
                    producer=producer,
                    field=input_name,
                    candidates=tuple(sorted(consumer_inputs)),
                )
            ...
            if producer not in dag.get_dependencies(node):
                raise AlignmentError(
                    ...
                    producer=producer,
                    field=input_name,
                    candidates=tuple(sorted(dag.get_dependencies(node))),
                )
        ...
        if full_input_cover:
            ...
            for input_name in consumer_inputs:
                ...
                raise AlignmentError(
                    ...
                    node=node,
                    field=input_name,
                    candidates=tuple(sorted(wired_inputs | static_keys)),
                )

        # G6 (opt-in): compile-time capability gate — a manifest's author-declared REQUIRED
        # capabilities (spec.requires) must all be present in the agent's own manifest.capabilities
        # inventory. Fails closed at compile with an answer-carrying error naming the missing labels.
        # Conservative + opt-in: an agent that requires nothing never trips, so default off (and even
        # on) is byte-for-byte the prior behavior for un-annotated manifests.
        if require_capabilities:
            required = _required_capabilities(manifest)
            declared = _declared_capabilities(manifest)
            for kind in _CAP_KINDS:
                missing = required[kind] - declared[kind]
                if missing:
                    raise AlignmentError(
                        f"{node}: manifest requires {kind} {sorted(missing)!r} but its "
                        f"capabilities.{kind} declares {sorted(declared[kind])!r} — a capability "
                        "gate violation (the target agent's runtime does not attest these)",
                        node=node,
                        field=kind,
                        expected=tuple(sorted(missing)),
                        candidates=tuple(sorted(declared[kind])),
                    )
