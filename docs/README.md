...
| [`execute`](reference/execute.md) | The `Supervisor` — topological dispatch over a frozen plan. *(opt-in: `Supervisor.run(parallel=N)` antichain wave, `NODE_EXECUTORS` dispatch seam, `check_acceptance`.)* | [`execute/`](../src/concursus/execute/) |
