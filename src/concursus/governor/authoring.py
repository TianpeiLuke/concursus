"""Net-new agent-manifest authoring ( Phase 3b) — the deepest form of Create.
...
def staff_capability_dag(
    ...
    *,
    ...
    """Turn an agent-agnostic CAPABILITY ``AgentDAG`` into an assemblable manifest set (
...
class RebindExhausted(ValueError):
    """Raised when a bounded re-bind cannot find a type-aligning agent assignment ( C2)."""
...
def staff_with_rebind(
    ...
    *,
    ...
    The compiler's regulator half: instead of hard-erroring when a bound team fails
