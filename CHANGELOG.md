...
Every change leaves the default `plan → deploy → run` path byte-for-byte unchanged;
concursus stays a **compiler, not a runtime governor** (single static `Supervisor.run`
over a frozen `plan.order`; resume = replay; all dynamism is bounded recompile / a
pre-compile step / the outer bounded governor loop). Full suite green (654 passed,
2 skipped — up from 510).

### Added — compiler contract
- `AgentManifest` gains optional `capabilities` (declared features/tools/egress) and an
  integer `contract_version`; `validate()` fails closed above `MAX_SUPPORTED_CONTRACT_VERSION`.
- `AgentDAG` gains an order-independent Tarjan-SCC cycle classifier (additive; the default
  still rejects cycles). `check_alignment` errors are now answer-carrying (offending field,
  expected type, valid producer set) with an opt-in compile-time capability gate.
- A build-enforced single-source contract test plus an append-only API-surface snapshot guard
  (`tests/api_surface.snapshot.json`) — any un-reviewed public-surface change fails the build.

### Added — durable state
- `rundb` self-heals: a `PRAGMA quick_check` failure drops and rebuilds the derived index from
  the append-only SSOT rather than raising. Forward-only `filevault` note `schema_version` +
  a run-dir heartbeat lockfile. `DeployLedger` records typed rejections + desired-vs-confirmed
  reconciliation.

### Added — executor
- `Supervisor.run(parallel=N)`: an opt-in bounded antichain-parallel wave (default `parallel=1`
  is the exact serial pass; results are byte-identical, verified on a diamond DAG). Resume now
  verifies a plan content-hash identity guard + suppresses re-appending replayed steps.
  Hold-vs-crash failure classification surfaced in `summary()`. Assemble gains an opt-in
  validate-and-retry budget hook bounded by the recompile budget.

### Added — governor & cockpit
- Pure `compute_schedule(state) -> Decision{dispatch, declined[reason]}`. A persisted
  per-`(source, entity)` fire-budget admission gate + provenance self-trigger guard +
  detection-mode enum. Opt-in episode-boundary approval gate + an injected `EventSink`.
- Read-only cockpit projections over the append-only log: snapshot-then-follow (replay-from-offset,
  no full reconcile), a frozen-DAG family-tree annotated with live status, and a per-node event bus.

## [0.4.5] - 2026-07-16

Adds the note-capture front + the compile-time payload contract. Full
...
- **Post-run note-capture front (`state/capture.py`) — persist a run's plan + payloads as slipbox
 ...
 opt-in, default byte-for-byte unchanged. (Phases 0/1/3). +12 tests (`test_capture.py`).
...
- **Compile-time payload contract + trust-tiered context (`ProvisioningPlan.payload_contract`).** The
 ...
 (a `depends_on` edge or a context key). All opt-in, default off. (Phases 0/2). +7 tests
...
- **`check_hive_contract()` — the agent↔Hive-layer boundary gate (B2 remainder).** Deepens the
 ...
 gate (checked first). (completes P6.2's agent↔Hive-layer half). +2 tests.
...
- **Cross-domain transfer validated end-to-end (the compounding claim).** A regression measuring
 ...
 win through the full `distill → retrieve → prime` loop. C4 (plan P5.3). +1 test.
...
- **Cross-domain precedent priming for the decomposer (`plan_from_goal(decompose=True,
 ...
 Realizes cross-domain transfer at the planner front ( C3, plan P5.2). +4 tests.
...
- **`governor.authoring.staff_with_rebind()` — re-bind on alignment failure (the compiler as
 ...
 combination aligns within the bound. This is the "weak regulator → real regulator" fix:
...
- **`GovernorLoop(decompose=True, bind_fn=…)` — the decompose→bind pipeline as the loop's live
 ...
 pipeline the critique called for now runs end-to-end through the governor loop.**
...
- **`Supervisor(check_acceptance=True)` + `check_acceptance()` — output-QA acceptance contract
 ...
 ``check_acceptance=False`` keeps the run byte-for-byte unchanged. Phase-6 compiler
...
- **`OrchestrationAssembler(single_writer=True)` / `check_alignment(..., single_writer=True)` —
 ...
 `single_writer=False` keeps behavior byte-for-byte unchanged. Phase-6 compiler contract
...
- **`OrchestrationAssembler(strict_types=True)` / `check_alignment(..., strict_types=True)` — deep
 ...
 the name-level gate byte-for-byte unchanged. First slice of the Phase-6 compiler
...
- **`concursus run --vault --lean-form`** — the on-disk `StateStore` emits authentic
  slipbox notes by default (`slipbox_form=True` — indexer-ingestible, with a `_run.md`
...
- **`FileVaultStateStore`** — a persistent, on-disk `StateStore` backend (no AWS), closing the
  ...
  copy never re-ingested. **Notes conform to the the analysis notes format** by default
...
- The on-disk notes stay the single source of truth; `RunGraph` / `RunIndex` remain the fast
 ...
 offline / air-gapped / CI / debuggable durability tier; for AgentCore-hosted runs a
 BYO EFS/S3 Files mount or the managed Memory log remain the aligned choices.
