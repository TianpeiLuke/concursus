...
| `build` | Renders runtime/container/IAM artifacts, applies trust gates, records content-addressed deployment history, and optionally provisions AWS resources. | Assembly synthesizes build entries, but the AWS/Docker actuator is optional. `ledger` is later read by governor registry. |
| `execute` | Executes a frozen plan: `Supervisor` overlays `AgentRef` outputs onto inputs, invokes nodes, validates results, and records them. Optional harnesses add backends, monitoring, artifacts, retries, parallel waves, and futility cancellation. | Consumes `assemble.ProvisioningPlan`, `core.resolve`, and `state`; it can execute an assembled plan directly, without deployment. It imports the reasoning package's fan-out-capacity helper as a shared utility. |
...
| `ui` | Renders an operator DAG/node-card/Kanban console and holds the shared status presentation vocabulary. | A presentation-only leaf: it consumes record dictionaries and deliberately does not import the state/store layer or enter execution control flow. |
...
| `ui` | `status.py` owns the common status vocabulary and style table; `console.py` renders the offline/live operator console from a record dictionary without reaching into test or state code. |

## Why the relationships are directional

The source establishes five important boundaries.

1. `core → assemble → execute` is the main execution direction. `OrchestrationAssembler.assemble()` validates and resolves before returning a value-type `ProvisioningPlan`; the supervisor reads that value and does not revise it.
2. `build` is a side branch, not a required hop before execution. Assembly asks `RuntimeBuilderFactory` for per-agent entries; actual IAM/ECR/AgentCore work is confined to `build.provision` and reached only by explicit deployment. An offline or pre-provisioned plan can proceed directly to `Supervisor.run`.
...
4. `reasoning → assemble` is strictly pre-compile. Deliberation resolves hypotheses, then emits a DAG for the normal compiler path. It never calls the supervisor. `execute.supervisor` does import the reasoning package's fan-out-capacity helper, so this is a small utility-level dependency rather than a reversed control-flow relationship.
...
6. When enabled, the governor starts a new bounded episode around this lifecycle. Transfer has two intentionally separated phases: it authors its terminal agent before assembly, then exports finished episodic notes at an outer boundary to an external knowledge-consolidation workflow. The UI is a read-only renderer of record dictionaries, outside the execution path.
