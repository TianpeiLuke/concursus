...
Design: it *reuses* the statestore marshalling seam rather than reinventing it —
...
# The run-ownership HEARTBEAT lease sidecar (distinct from the per-write ``.lock``/``.gen`` OCC guard
# above, which serializes concurrent WRITES but never expresses "who owns this run"). A single
# supervisor stamps ``host:pid:epoch`` here to claim a run dir; a peer treats a lease as stale — and
# therefore reclaimable — when its pid is dead on this host OR its heartbeat has expired. Opt-in: the
# StateStore write path never touches it; only a supervisor that wants single-owner exclusion does.
_HEARTBEAT_NAME = ".concursus.heartbeat"
_HEARTBEAT_TTL_SECONDS = 30.0  # a lease older than this (no refresh) is presumed dead / reclaimable

# The vault posture: by DEFAULT the store emits notes conformant to the slipbox format
...
# The on-disk note format is versioned so it can evolve WITHOUT breaking a resume over older notes.
# The current schema is v1; an ABSENT ``schema_version`` on a note == v1 (the baseline), so every
# note written before this field existed reads correctly and the DEFAULT write path stays
# byte-identical (the stamp is opt-in behind ``stamp_schema_version``). Evolution is FORWARD-ONLY and
# applied on READ: the append-only log on disk is never rewritten/downgraded — a note is upgraded to
# the current shape in-memory as it is parsed. A future schema bump raises ``_NOTE_SCHEMA_VERSION``
# and registers its upgrade hook in ``_NOTE_MIGRATIONS``; while v1 is current the registry is empty
# (nothing to migrate — the read path is a no-op).
_NOTE_SCHEMA_VERSION = 1
_NOTE_SCHEMA_KEY = "schema_version"

#: Ordered forward-only migrations, keyed by the FROM version each upgrades. A hook at key ``n`` maps
#: a v``n`` record-field dict (the reconstructed ``meta``) to its v``n+1`` shape. Empty while v1 is
#: current. Never contains a downgrade (forward-only).
_NOTE_MIGRATIONS: Dict[int, Callable[[Dict[str, Any]], Dict[str, Any]]] = {}


def _note_schema_version(flat: Mapping[str, Any]) -> int:
    """The declared ``schema_version`` of a parsed note; ABSENT or malformed == v1 (the baseline)."""
    raw = flat.get(_NOTE_SCHEMA_KEY)
    if raw is None:
        return 1
    try:
        return int(raw)
...
def _migrate_note_meta(meta: Dict[str, Any], from_version: int) -> Dict[str, Any]:
    """Upgrade a parsed note's record-field ``meta`` from ``from_version`` to ``_NOTE_SCHEMA_VERSION``.

    Applies the ordered :data:`_NOTE_MIGRATIONS` hooks one version at a time (forward-only). Returns
    ``meta`` unchanged when the note is already current (the common case — nothing registered while
    v1 is current, so this is a no-op and the round-trip stays byte-exact). Read-time projection: it
    never rewrites or downgrades the on-disk note.
    """
    version = from_version
    if version >= _NOTE_SCHEMA_VERSION:
        return meta
    upgraded = dict(meta)
    while version < _NOTE_SCHEMA_VERSION:
        migrate = _NOTE_MIGRATIONS.get(version)
        if migrate is None:
            break  # no forward path from here — stop rather than guess (defensive)
...
def _record_to_note(
    ...
    * ``slipbox_form=True`` (default) emits a note conformant to the slipbox format —
    ...
    ``stamp_schema_version`` (opt-in, default off) stamps a ``schema_version`` frontmatter key so a
    ...
    if not slipbox_form:
        ...
        if stamp_schema_version:
            lines.append(f"{_NOTE_SCHEMA_KEY}: {json.dumps(_NOTE_SCHEMA_VERSION)}")
    ...
    if stamp_schema_version:
        fm[_NOTE_SCHEMA_KEY] = _NOTE_SCHEMA_VERSION  # opt-in migration stamp (off = byte-identical)
...
def _note_to_record(text: str) -> Record:
    ...
    Forward-only schema evolution is applied HERE, on read: the note's ``schema_version`` (ABSENT ==
    v1, the baseline) selects the ordered :data:`_NOTE_MIGRATIONS` hooks that upgrade the
    reconstructed ``meta`` to :data:`_NOTE_SCHEMA_VERSION` before it is marshalled. While v1 is
    current the registry is empty, so this is a no-op and the round-trip stays byte-exact.
    """
    ...
    # Forward-only note-schema evolution: an older on-disk note is upgraded to the current shape in
    # memory as it is parsed (the log is never rewritten). ABSENT schema_version == v1 (baseline);
    # the migration is a no-op while v1 is current, so a current-version note round-trips byte-exact.
    version = _note_schema_version(flat)
    if version < _NOTE_SCHEMA_VERSION:
        meta = _migrate_note_meta(dict(meta), version)
    node = meta.get("node", "")
...
def redact(payload: Mapping[str, Any], *, deny: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    the counter + flagged for persisting payloads.
    """
...
# on :class:`_Critical` (which serializes concurrent writers but never says who OWNS the run): a crash
# leaves the OCC ``.lock`` free the instant the process dies, whereas this lease survives on disk so a
# would-be new owner can distinguish "another supervisor is alive and refreshing" from "the prior
# owner is dead — reclaim". It is entirely OPT-IN: nothing in the default StateStore write/resume path
# touches it; a supervisor constructs a :class:`RunHeartbeatLock` explicitly when it wants the guard.
def _pid_alive(pid: int) -> bool:
    """Whether local ``pid`` is a live process (best-effort, POSIX ``kill(pid, 0)``).

    Returns ``True`` when liveness cannot be determined (unknown pid <= 0, or a platform without
    ``os.kill``) — a fail-safe that biases toward NOT stealing a possibly-live owner's lease.
    """
    if pid <= 0:
        return True
    try:
        ...
        return True  # can't tell (non-POSIX / no os.kill) — assume alive, don't steal
    return True


def _parse_heartbeat(text: str) -> Optional[Tuple[str, int, float]]:
    """Parse a ``host:pid:epoch`` lease line into ``(host, pid, epoch)``; ``None`` if malformed."""
    parts = text.strip().split(":")
    if len(parts) != 3:
        return None
    host, pid_s, epoch_s = parts
    try:
        return host, int(pid_s), float(epoch_s)
    except (TypeError, ValueError):
        return None


class RunHeartbeatLock:
    """An **opt-in** ``host:pid:epoch`` run-ownership lease at the run-dir root (supervisor-only).

    ``acquire`` claims the run dir for this ``host:pid`` unless a NON-stale lease held by a different
    owner already exists (then it raises :class:`RunLockHeldError`). ``refresh`` re-stamps the epoch
    to keep the lease live (call it periodically from the run loop). ``release`` drops our lease.

    A lease is **stale — and therefore reclaimable** — when either its pid is dead on THIS host, or
    its heartbeat epoch is older than ``ttl_seconds`` (a supervisor that died mid-run, or one that
    stopped refreshing, no longer owns the run). A lease held by a live, freshly-refreshed peer is
    honoured. This is a cooperative advisory lease — it does not use OS file locks, so it works
    cross-host over a shared vault where ``fcntl`` would not; it is intentionally separate from the
    per-write ``.lock``/``.gen`` OCC guard and NEVER engaged by the default StateStore path.

    Usable as a context manager (acquires on enter, releases on exit). All operations are pure
    stdlib + atomic writes; no boto3, no network.
    """

    def __init__(self, run_dir, *, ttl_seconds: float = _HEARTBEAT_TTL_SECONDS, host: Optional[str] = None,
                 pid: Optional[int] = None) -> None:
        self._dir = Path(run_dir)
        self._path = self._dir / _HEARTBEAT_NAME
    ...
    @property
    def path(self) -> Path:
        """The on-disk lease sidecar path (``<run_dir>/.concursus.heartbeat``)."""
        return self._path

    @property
    def owner_token(self) -> str:
        """This lock's ``host:pid`` owner identity (the epoch is stamped per refresh)."""
        return f"{self._host}:{self._pid}"

    def _stamp(self) -> None:
        self._dir.mkdir(parents=True, exist_ok=True)
        line = f"{self._host}:{self._pid}:{time.time()!r}"
        FileVaultStateStore._atomic_write(self._path, line)

    def read(self) -> Optional[Tuple[str, int, float]]:
        """The current on-disk lease as ``(host, pid, epoch)``, or ``None`` if absent/malformed."""
        try:
            text = self._path.read_text(encoding="utf-8")
        except (OSError, ValueError):
            return None
        return _parse_heartbeat(text)

    def _is_stale(self, lease: Tuple[str, int, float], *, now: float) -> bool:
        """A lease is reclaimable if its heartbeat expired OR its pid is dead on this same host."""
        host, pid, epoch = lease
        if (now - epoch) > self._ttl:
            return True  # heartbeat expired — presumed dead / no longer refreshing
        if host == self._host and not _pid_alive(pid):
            return True  # same-host owner process is gone — reclaim
        return False

    def is_held_by_other(self, *, now: Optional[float] = None) -> bool:
        """Whether a NON-stale lease owned by a DIFFERENT ``host:pid`` currently holds the run."""
        lease = self.read()
        if lease is None:
            return False
        host, pid, _ = lease
        if (host, pid) == (self._host, self._pid):
            return False  # our own lease
        return not self._is_stale(lease, now=now if now is not None else time.time())

    def acquire(self, *, now: Optional[float] = None) -> "RunHeartbeatLock":
        """Claim the run dir; reclaim a stale lease. Raise :class:`RunLockHeldError` if a live peer holds it."""
        if self.is_held_by_other(now=now):
            lease = self.read()
            raise RunLockHeldError(
                f"run {self._dir} is held by a live lease {lease!r}; not this owner {self.owner_token!r}"
            )
        self._stamp()
        self._held = True
        return self
    ...
    def refresh(self) -> None:
        """Re-stamp the lease epoch to keep it live (raise if we never acquired / no longer own it)."""
        lease = self.read()
        if lease is not None and (lease[0], lease[1]) != (self._host, self._pid):
            raise RunLockHeldError(
                f"cannot refresh: run {self._dir} lease {lease!r} is not owned by {self.owner_token!r}"
            )
        self._stamp()
        self._held = True

    def release(self) -> None:
        """Drop our lease (a no-op if it is absent or already owned by someone else)."""
        lease = self.read()
        if lease is not None and (lease[0], lease[1]) == (self._host, self._pid):
    ...
    def __enter__(self) -> "RunHeartbeatLock":
        return self.acquire()
...
class RunLockHeldError(RuntimeError):
    """Raised when a :class:`RunHeartbeatLock` cannot claim a run held by a live, non-stale owner."""
