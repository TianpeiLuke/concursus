...
class EventSink(Protocol):
    """OPT-IN observability seam: receive episode-BOUNDARY events, never mid-episode.
    ...
    :class:`NullEventSink` is the canonical explicit no-op. Any exception raised by ``emit`` is
...
class NullEventSink:
    """The default no-op :class:`EventSink` — drops every event.

    Passing this is behaviorally identical to leaving ``event_sink`` unset (None): a run with a
    NullEventSink returns a byte-identical :class:`GovernorResult`, because emitting an event never
...
class GovernorLoop:
    ...
    def __init__(
        ...
        *,
        ...
        # P3a: OPT-IN auto-Create. When ``auto_create`` and a scheduler are set, the
        ...
        # -- OPT-IN episode-boundary approval/interrupt gate (default OFF) --
        # None (default) => today's behavior byte-for-byte: no gate is ever consulted, the loop runs
        # every bounded round with no interruption. When a callable is provided, it is consulted ONLY
        # at EPISODE BOUNDARIES (before each Supervisor episode is dispatched, never mid-episode; the
        # supervisor still runs as a single static pass, INV-1). It receives a read-only VALUE view of
        # the boundary (``{"type": "episode_boundary", "run_id", "round", "completed", "frontier"}``)
        # and returns a verdict: falsy / "approve" / "continue" => dispatch the episode as normal;
        # "abort" (or "stop"/"halt") => stop the loop BETWEEN episodes (no episode runs this round),
        # finalizing with ``terminated_by="aborted"``; "pause"/"hold" => same bounded stop but labeled
        # ``paused`` (a warm-resumable boundary, since nothing was dispatched). The gate can only STOP
        # the bounded loop earlier — it can never extend it past the hard bounds, mutate a plan, or
        # reach inside a running Supervisor (INV-1/INV-2/INV-3).
        self._episode_gate = episode_gate
        # -- OPT-IN episode-boundary event sink (default no-op) -------------
        # None (default) => NullEventSink semantics: nothing is emitted, no observer is called, the
        # GovernorResult is byte-identical. When a sink is provided, the loop emits plain-dict VALUE
        # events at episode boundaries only (episode_start / episode_end / decision) — never a live
        # ctx/plan handle, so an observer can never mutate the run (INV-3/INV-5). Emission is wrapped
        # so a misbehaving sink can never break a live episode.
    ...
    def _boundary_view(self, ctx: dict, *, event_type: str) -> Dict[str, Any]:
        ...
        an event sink can reach inside the run or mutate it (INV-3/INV-5). Deriving from the log (not
        the yet-unpopulated ``ctx["frontier"]``) makes the FIRST boundary — consulted before any
        episode runs — carry the true still-open frontier. Emitted/consulted at episode boundaries
        only, never mid-episode.
        """
        completed = sorted(self._store.completed())
        completed_set = set(completed)
        plan = ctx.get("plan")
        order = list(getattr(plan, "order", []) or [])
        frontier = [n for n in order if n not in completed_set]
        return {
            "type": event_type,
            "run_id": self._run_id,
            "round": int(ctx.get("round", 0)),
    ...
    def _consult_gate(self, ctx: dict) -> Optional[str]:
        """Consult the OPT-IN episode-boundary gate; return a normalized stop label or ``None``.

        No-op (returns ``None``) when no gate is configured — the default path never calls a gate, so
        behavior is byte-for-byte unchanged. When a gate IS set it is invoked with a read-only
        boundary VALUE (never the live ctx) at THIS episode boundary, strictly BEFORE any supervisor
        is built or dispatched (never mid-episode; INV-1). A falsy / ``approve`` / ``continue``
        verdict returns ``None`` (dispatch normally); ``abort``/``stop``/``halt`` returns
        ``"aborted"`` and ``pause``/``hold`` returns ``"paused"`` — both stop the bounded loop
        between episodes without running one. Any gate exception is swallowed (fail-open: the loop
        continues, degrading to the ungated bounded default) so a buggy gate can never break a run.
        """
        gate = self._episode_gate
        if gate is None:
            return None
        try:
            verdict = gate(self._boundary_view(ctx, event_type="episode_boundary"))
        except Exception:  # noqa: BLE001 - a gate error must never break the bounded loop
            return None
        if not verdict:
            return None
        v = str(verdict).strip().lower()
        if v in ("abort", "stop", "halt"):
            return "aborted"
        if v in ("pause", "hold"):
            return "paused"
        return None
    ...
    def _run_episode(self, ctx: dict) -> dict:
        ...
        OPT-IN episode-boundary gate (default OFF): before ANY supervisor is built or dispatched,
        the injected ``episode_gate`` (if any) is consulted at this EPISODE BOUNDARY — never
        mid-episode. A stop verdict (``abort``/``pause``) sets ``ctx["gate_stop"]`` and returns
        WITHOUT running an episode (no supervisor is constructed, no node is invoked, the round
        counter is NOT bumped, the frozen plan is untouched — INV-1/INV-3); ``collect`` then
        ...
        if gate_stop:
            ctx["gate_stop"] = gate_stop
            ctx["trace"].append("run_episode")
            return ctx
        self._emit_event("episode_start", ctx)
        factory_kwargs = dict(
            plan=ctx["plan"],
            manifests=self._active_manifests(),
            store=self._store,
    ...
    def _collect(self, ctx: dict) -> dict:
        ...
        OPT-IN gate short-circuit (default OFF): if the episode-boundary gate stopped this round
        (``ctx["gate_stop"]`` set by :meth:`_run_episode`), NO episode ran — there are no new outputs
        to fold. Collect then leaves the log/frontier exactly as the prior round left them, records
        no progress, and defers to the routing edge (which reads ``gate_stop`` and terminates). This
        branch is never entered on the default (no-gate) path.
        """
        if ctx.get("gate_stop"):
            # No episode was dispatched this boundary — re-derive read-only state from the log and
            # return WITHOUT folding outputs or mutating the plan. The routing edge finalizes.
            completed = self._store.completed()
            ctx["completed"] = sorted(completed)
            ctx["frontier"] = [n for n in ctx["plan"].order if n not in completed]
        ...
        ctx["trace"].append("collect")
        # OPT-IN observability: emit the episode-END boundary event AFTER the log fold + progress
        # bookkeeping (a read-only VALUE; no-op absent a sink; INV-5).
        self._emit_event("episode_end", ctx, done=bool(ctx["done"]), progressed=progressed)
        return ctx
    ...
    def _route_after_collect(self, ctx: dict) -> str:
        ...
        OPT-IN gate short-circuit (default OFF): if the episode-boundary gate stopped the round
        (``ctx["gate_stop"]``), this terminates BETWEEN episodes at ``synthesize`` with the gate's
        label (``aborted``/``paused``), checked FIRST so it can only stop the bounded loop earlier —
        never extend it past the hard bounds.
        """
        route = self._decide_route(ctx)
        # OPT-IN observability: emit the routing DECISION as an episode-boundary event (read-only
        # VALUE; no-op absent a sink). Carries the chosen route + the terminal label (if any).
        self._emit_event(
            "decision", ctx, route=route, terminated_by=ctx.get("terminated_by") or None
        )
        return route

    def _decide_route(self, ctx: dict) -> str:
        """The pure routing verdict (``planner`` | ``synthesize``); default path byte-for-byte today."""
        # OPT-IN gate stop: an episode-boundary abort/pause finalizes between episodes.
        if ctx.get("gate_stop"):
            ctx["terminated_by"] = str(ctx["gate_stop"])
            return "synthesize"
